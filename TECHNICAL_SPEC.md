...
23. Строковое указание пользователя не читать, не открывать или не показывать
    ...
    том же запросе.
24. Один ограниченный JSON-блок `<acceptance_manifest>` версии 1 задаёт точные
    количества tools, ordered required events, forbidden events, разрешённые
    статусы и внешние pending-проверки. Неизвестные поля, tools, статусы,
    зависимости или превышение лимитов делают manifest невалидным. Tool,
    фактически вызванный без объявленного exact count, является нарушением.
