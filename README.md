...
Версия **0.33.0**. Явное продолжение, безопасная связанная проверка и
долговечная защита от повторного запуска. Доказательства и границы приёмки:
[отчёт 0.33](EXPLICIT_RESUME_ACCEPTANCE.md).
Это локальный агент для доверенных проектов, не OS sandbox для враждебного кода.

## Продолжение сохранённой задачи — 0.33

В чате откройте **Действия задачи / Task actions**, выберите сохранённую задачу
и нажмите **Продолжить / Resume** или **Продолжить проверки / Resume verification**.
Это типизированное действие: семантический классификатор не решает, разрешено ли
нажатие. Сервер всё равно проверяет принадлежность, ревизию, аренду и права.

Для старой задачи с неподтверждённой связью выполнения используйте **Сверить
состояние / Reconcile**. Если безопасное продолжение нельзя доказать, задайте
корень, например `/workspace/ozon_market_analytics`, и подтвердите **Создать
связанную проверку / New verification**. В корне должен быть `pyproject.toml`;
проверка использует валидированное окружение проекта. Источник остаётся
неизменным, неизвестные записи не повторяются. Ошибки исправляются только через
отдельное подтверждённое действие **Create repair task**.

Повтор действия после обновления страницы возвращает тот же execution. Если
сервер завершился между сохранением действия и отправкой worker, UI показывает
неподтверждённый запуск и доступное восстановление, а не запускает второй worker.
Глобальные правила дополнены [ТЗ](EXPLICIT_RESUME_TECHNICAL_SPEC.md) и
[блочным промптом](EXPLICIT_RESUME_IMPLEMENTATION_PROMPT.md).
...
длинная работа делает soft yield и продолжается следующей unit без повторного
inventory и replay подтверждённых изменений. Несколько разных страниц одного
файла разрешены в пределах общего byte/page budget; полный повтор уже покрытого
диапазона блокируется. Parent Web task связан с дочерними model attempts.

В Web-чате индикатор памяти показывает фактическое состояние: `FTS5/BM25`, а для
локального vector слоя — `lazy`, `ready`, `degraded` или `disabled`. Hybrid search
означает RRF-объединение SQLite BM25 и FastEmbed/Qdrant; при локальном сбое вектора
лексический поиск продолжает работу. Сам индикатор не загружает embedding model.

Во вкладке Providers можно атомарно сохранить в `AGENT_DATA_DIR/model-routing.json`
адаптивные профили `fast/standard/reasoning`, cost/latency/local-only и пределы
execution escalation. Ключи туда не записываются, текущий запрос не переключается.
Без профилей остаётся видимый `configured-chain`. Reasoning/frontier model —
ресурс для сложных или рискованных запросов, а не default и не источник прав.

Размер входа оценивается единым Unicode-aware estimator с резервом вывода. Это
консервативная оценка маршрутизации, не billing usage. Опциональный tokenizer
включается `AGENT_TOKEN_ESTIMATOR=tiktoken`; любой его сбой безопасно возвращает
offline fallback. Качество retrieval можно измерять без LLM-судьи через
`context_agent.retrieval_eval` (hit@k и MRR по natural/lexical golden corpus).

Воспроизводимый bounded-progress live-тест выполняет реальные LLM-вызовы в двух
изолированных workspace/SQLite и проверяет soft yield, restart/resume, 14 чтений,
пять страниц одного файла и exact-once запись:
`python scripts/live_bounded_progress.py --repeat 2`.

## Продолжение задачи и выбор модели — 0.25.0

Разработка больше не начинается с обязательного полного аудита. В чате Agent
с автоматическим исполнением сохраняет исходную задачу, план, следующий шаг и
отдельные подтверждения инструментов в SQLite. Команда «продолжи выполнение задачи
в соответствии с тз и промтом» восстанавливает ту же задачу и persistent job даже
после анализа лога или перезапуска приложения. Для восстановления нужны прежние
`AGENT_DATA_DIR`, workspace и thread; новая БД не содержит старого плана.

- Полный manifest создаётся только для явно выбранного `project-audit`.
  `project-change` и `project-test` выполняются короткими execute units.
- При нескольких незавершённых задачах выберите нужную в чате. Неоднозначная
  формулировка может вызвать ограниченный JSON-классификатор; сбой не запускает
  новый аудит и не выдаёт разрешений. Отключение: `AGENT_SEMANTIC_ROUTING=false`.
- Ask, Plan, текущий запрет записи и явное «ничего не меняй» сохраняют приоритет.
  Исторический `Workspace access denied` не считается вечным запретом.
- `save_task_checkpoint` сохраняет план между units. Если следующий шаг задан
  и `pause=false`, исполнение продолжается в пределах лимитов. При паузе, отсутствии
  следующего шага или исчерпании бюджета прогресс сохраняется. Конец ответа модели
  не доказывает готовность проекта: общую задачу закрывает оператор.
- CLI `job --allow-write` использует тот же resolver и persistent units;
  `ask` и интерактивный CLI `chat` остаются одноходовыми, но умеют восстановить
  задачу и её права. CLI `audit` предназначен именно для полного аудита.

В заголовке Web-чата доступны «Авто по задаче / Adaptive» и «Выбранная модель /
Manual». Изменение provider/model или выбор модельного пресета включает Manual;
выбор auto-пресета возвращает Adaptive. Настройка режима запоминается браузером.
Уже выполняющийся запрос не переключается задним числом. Согласованный резерв
сохраняется, но не может обойти ограничения приватности и возможностей модели.

Для адаптивного выбора задайте `AGENT_MODEL_PROFILES` как JSON-массив в одну строку
в `.env.local`. Каждый профиль содержит:

| Поле | Назначение |
| --- | --- |
| `provider`, `model` | ID включённого провайдера и реально доступной модели; для GLM provider — `zhipu` |
| `tier` | `fast`, `standard` или `reasoning` |
| `context_tokens` | Проверенный оператором предел контекста, включая резерв вывода |
| `tools` | Поддержка инструментов, true/false |
| `input_usd_per_million`, `output_usd_per_million` | Необязательные тарифы; отсутствие — неизвестно, не ноль |
| `latency_ms` | Необязательная ожидаемая задержка |
| `quality` | Операторская оценка 0..1, не результат автоматического бенчмарка |
| `enabled` | Включение профиля, по умолчанию true |

Не подставляйте выдуманные ID/тарифы. Для каждой используемой категории нужен
подходящий профиль: высокий риск или сложность → reasoning; простой запрос без
tools → fast; остальные → standard. Safety-разрешения не зависят от tier. Между
кандидатами учитываются настроенное качество, наблюдаемая транспортная надёжность,
оценка стоимости и задержка. История ответов не выдаётся за оценку их правильности.

Без профилей применяется прежняя цепочка с причиной `PROFILES_NOT_CONFIGURED`,
а не скрытая «оптимизация». Локальный `.env.local` автоматически не переписывается.
Стоимость ограничивается параметром `AGENT_MODEL_COST_LIMIT_USD` (0 — выключено),
ожидаемая latency — `AGENT_MODEL_LATENCY_BUDGET_MS`. При заданном ограничении
неизвестные тарифы/latency не проходят фильтр. Размер входа оценивается единым
Unicode-aware estimator, включая schemas tools, checkpoint и safety reserve. Это не
точный provider tokenizer/billing;
latency фильтр не гарантирует срок ответа удалённого API. Общие timeout/attempt
лимиты продолжают действовать. `AGENT_MODEL_LOCAL_ONLY=true` исключает внешние
LLM-вызовы, включая semantic classifier и fallback; допустим только loopback
LM Studio. Отдельные веб-инструменты этой настройкой не отключаются.

Полные требования и порядок реализации:
[ТЗ 0.25](TASK_RESUME_MODEL_ROUTING_SPEC.md),
[промт 0.25](DEEP_CONTEXT_AGENT_0_25_RESUME_MODEL_ROUTING_PROMPT.md).
Воспроизводимый live-тест (платные API-вызовы, отдельный временный workspace/БД):
`python scripts/live_resume_routing.py`; отдельно классификатор:
`python scripts/live_resume_routing.py --classifier-only`.
...
Там же расположен индикатор фактически доступной памяти. Во вкладке провайдеров
редактируется общая adaptive policy: включение, профили tier, cost/latency,
local-only и ограниченная escalation. Политика применяется со следующего запроса
и переживает перезапуск; API-ключи остаются только в серверном окружении.
...
Версия 0.24.0 предназначена для эксплуатации как локальный
...
### Продолжение задач и модели (0.24)

Команда «продолжи» в том же чате/workspace восстанавливает сохранённую задачу.
Анализ вставленного лога не меняет цель и не разрешает массовый обход проекта.
Если незавершённых задач несколько, выберите одну в «Продолжить задачу»;
ненужную задачу можно явно закрыть/отменить. Ask/Plan и выключенный allow-write
сохраняют запрет правок. Успешный одиночный этап имеет статус partial до явного
закрытия задачи или проверенного завершения persistent job.

Списки моделей в чате и провайдерах содержат до пяти последних вариантов.
Дата релиза берётся из официальных публикаций/полей API, иначе подписывается
дата записи каталога. Даты без источника не выдумываются; alias и snapshot не
дублируются. Старая активная модель сохраняется, а не переключается автоматически.
Проверка доступности каталога не означает успешный inference всех моделей.

Diagnostics schema 3 хранит фактические model attempts, usage (null если неизвестно),
finish/stop reason, retry и отмену отдельно от rollback истории. Управление бюджетами:
`AGENT_MODEL_OUTPUT_TOKENS`, `AGENT_TASK_MODEL_ATTEMPTS`, `AGENT_TASK_TIMEOUT_SECONDS`,
`AGENT_NO_PROGRESS_LIMIT`, `AGENT_REPETITION_MODE=off|observe|enforce`.
Детектор повторов не потоковый и не семантический; по умолчанию только observe.
Локальная отмена не гарантирует прекращение inference/оплаты на сервере провайдера.
После аварии lease старого task owner истекает в пределах бюджета запуска + 60 с.

Отдельные live-проверки (используют существующие ключи, могут расходовать API-квоту):

```powershell
.\.venv\Scripts\python.exe scripts/live_task_continuity.py
.\.venv\Scripts\python.exe scripts/live_model_catalog.py
```

Первый сценарий работает только с новой временной папкой и отдельными SQLite,
второй читает каталоги моделей и не выполняет inference.
