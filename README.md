...
- Явное указание «не читай/не открывай/не показывай PATH» или английский
  ...
  блокирует `read_file` до обращения к backend.
...
Версия 0.4.0 готова для production-эксплуатации как локальный однопользовательский
...
`PASS`/`FAIL`. Pytest остаётся внешней проверкой качества кода.
