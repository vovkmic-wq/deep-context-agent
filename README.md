...
- Runtime разрешает не более одного tool call на model step и передаёт
  ...
  мутация в одном пользовательском ходе отклоняется как `denied`.
...
Версия 0.3.0 готова для production-эксплуатации как локальный однопользовательский
...
самой LLM, всегда остаётся `NOT_VERIFIED` либо становится `FAIL` при ошибках в
фактическом audit.
