...
- Autopilot перенесён в обычный Web-чат; отдельная вкладка и дублирующая форма
  удалены.
- Чат получил явные режимы `auto`, `autopilot`, `single-turn`; engineering
  cards сразу выбирают Autopilot, а выбор пользователя сохраняется локально.
- Фраза «разбираться с недостающими модулями проекта по промту и ТЗ» теперь
  распознаётся как длительная задача. Если неизвестная формулировка всё же
  достигает `agent_step_limit`, auto mode откатывает ход и продолжает той же
  целью в persistent job.
- `execution`, `job_progress`, `job_replanned` и `job_verification` отображаются
  прямо в сообщении чата; job history находится в боковой панели.
- Завершённый Autopilot-turn архивирует user/assistant сообщения в исходный
  thread, а роли `user` и `human` одинаково корректно отображаются после reload.

### Безопасность

- Текст задачи по-прежнему не повышает read-only до allow-write; право записи
  передаётся только отдельным trusted полем чата.
