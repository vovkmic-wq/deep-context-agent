...
## Явное продолжение — 0.33
Применять R1–R6 EXPLICIT_RESUME_TECHNICAL_SPEC.md: действия с выбранной задачей
передаются структурированно; статус и разрешённые действия принадлежат runtime.
Закреплённый заголовок содержит режим/provider/model; дополнительные действия
сворачиваются, не вытесняя переписку и поле ввода. UI сохраняет idempotency key
при повторе, не очищает черновик, показывает неподтверждённый dispatch без ложного
«выполняется». Новая связанная проверка требует явно указанного root и подтверждения.
Reload/выбор задачи не выполняет POST/REPAIR автоматически.
